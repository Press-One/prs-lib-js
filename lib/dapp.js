...
DApp.prototype.webAuthorize = async function(
  appAddress,
  ...
  if (authOpts.privateKey) {
    authorizeOpts["signLocation"] = "client";
    ...
    authorizeOpts["signLocation"] = "server";
  }
 ...
 * @returns {Promise}
 */
DApp.prototype.authenticate = async function(appAddress, authAddress, hashAlg) {
  validator.assert(appAddress, "appAddress cannot be null");
  validator.assert(
    this.config.privateKey || this.config.token,
    "config.privateKey or config.token cannot be null"
  ...
  if (authAddress) {
    ...
    let sign = null;
    if (authOpts.privateKey) {
      sign = utility.signBlockData(blockData, authOpts.privateKey, hashAlg);
    } else if (authOpts.token) {
 ...
 * @returns {Promise}
 */
DApp.prototype.deauthenticate = async function(
  appAddress,
  authAddress,
  hashAlg = signUtility.ALG.KECCAK256
) {
