...
File.prototype.signByFileReader = async function(
  ...
  if (authOpts.privateKey) {
    sign = utility.signBlockData(blockData, authOpts.privateKey, data.hashAlg);
...
File.prototype.signByBuffer = async function(data, meta) {
  ...
  const fileHash = signUtility.hashBuffer(data.buffer, data.hashAlg);
  let blockData = {
    file_hash: fileHash
  };
  let sign = null;
