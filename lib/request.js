...
const hashRequest = (path, payload, hashAlg = signUtility.ALG.KECCAK256) => {
  // calculate hash
  ...
  return utility.hashText(dataStr, hashAlg);
};
...
const signRequest = (
  ...
  payload,
  privateKey,
  hashAlg = signUtility.ALG.KECCAK256
) => utility.signHash(hashRequest(path, payload, hashAlg), privateKey);
...
const getAuthHeader = (
  ...
  // sign hash
  const { signature, hash } = signRequest(path, payload, privateKey, hashAlg);
...
const createApiUrl = ({ host, version = "v2", path }) => {
  let apiURL = host;
...
const request = ({
  ...
  if (authOpts && authOpts.privateKey) {
    ...
    headers = Object.assign(
      {},
      headers,
      getAuthHeader(path, payload || undefined, authOpts.privateKey)
    );
  } else if (authOpts && authOpts.token) {
    headers = Object.assign({}, headers, {
      authorization: `Bearer ${authOpts.token}`
    });
  }
