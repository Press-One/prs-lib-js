...
describe("File", function() {
  ...
  it("reward", async function() {
    try {
      const buyClient = new PRS({
        env: prsEnv,
        debug: true,
        privateKey: utility.recoverPrivateKey(buyer.keystore, buyer.password),
        address: buyer.address
      });
      const res = await buyClient.file.reward(fileRId, 1, {
        comment: "comment",
        memo: "memo"
      });
      res.status.should.equal(200);
  ...
  it("get files", async function() {
    try {
      const res = await client.file.getFeeds(user.address, {
        offset: 0,
        limit: 10
      });
      res.status.should.equal(200);
